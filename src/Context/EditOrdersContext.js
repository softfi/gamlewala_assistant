import { createContext, useState } from "react";

// creating context api
export const EditOrdersContext = createContext(null);

// creting context api wrapper to wrap child components
const EditOrdersContextWrapper = (props) => {

    // declaring state for login status
    const [id, setId] = useState(false);

    return (
        // passing login state getter and setter as context value
        <EditOrdersContext.Provider value={[id, setId]}>
            {/* wrapping up child components */}
            {props.children}
        </EditOrdersContext.Provider>
    );
};

export default EditOrdersContextWrapper;