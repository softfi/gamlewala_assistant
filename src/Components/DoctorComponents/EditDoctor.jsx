...
const EditDoctor = () => {
  ...
  useEffect(() => {
    ...
    GetData({ url: `web/doctor/${params.id}`, token: token })
      .then((res) => {
        setSingleDoctorData(res.data.data);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  useEffect(() => {
    if (singleDoctorData) {
      setEditableImg(singleDoctorData.image);
      setDetails((prevDetails) => ({
        ...prevDetails,
        name: singleDoctorData.name,
        patientHandled: singleDoctorData.patientHandled,
        experties: singleDoctorData.experties,
        shortDescription: singleDoctorData.shortDescription,
        description: singleDoctorData.description,
      }));
    }
  }, [singleDoctorData]);
  ...
  const update = async (e) => {
    ...
    EditData({ url: "web/doctor", cred: credentials, token: token })
      .then((res) => {
        window.scrollTo(0, 0);
        setAlert({
          successStatus: true,
