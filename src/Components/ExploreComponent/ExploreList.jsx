...
const ExploreList = () => {
  ...
  useEffect(() => {
    ...
    GetData({ url: "explore", token: token })
      .then((res) => {
  ...
  const deleteItem = (id) => {
    let token = getSession("authorization");
    DeleteData({ url: `explore/${id}`, token: token })
      .then((res) => {
        window.scrollTo(0, 0);
        if (res.data.status) {
          setAlert({
      ...
      render: (_, elem) => (
        <Link href="app-product.html" className="me-4">
          <div className="sa-symbol sa-symbol--shape--rounded sa-symbol--size--lg">
            <img src={elem.upload} width="40" height="40" alt="" />
          </div>
        </Link>
      ),
    },
